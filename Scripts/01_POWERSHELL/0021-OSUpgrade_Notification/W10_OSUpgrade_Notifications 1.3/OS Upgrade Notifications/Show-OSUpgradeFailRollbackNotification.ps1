# Script to display a custom message to the user upon failure of the Windows 10 setup in an OS upgrade task sequence.
# For rollback only
...
#[int64]$Code = 3247440384
$hex = ('{0:x}' -f $Code).ToUpper()
$hex = "0x" + $hex

# Display the message
$Message = "The upgrade to Windows 10 x64 1803 could not complete. Your system has been rolled back to the previous version of Windows. Windows setup returned the following error code:&#10;&#10;$hex&#10;&#10;Please contact Helpdesk for further assistance."
New-WPFMessageBox -Content $Message -ContentTextForeground Black -ContentFontWeight DemiBold -ContentFontSize 16 -Title "OS Upgrade Failure!" -TitleFontSize 24 -TitleBackground Red -TitleTextForeground White -Sound 'Windows Exclamation'