# Script to display a custom message to the user upon successful completion of an OS upgrade task sequence.

Param(
    [String]$WhatsNewURL1 = "https://blogs.windows.com/windowsexperience/2018/04/30/whats-new-in-the-windows-10-april-2018-update/",
    [String]$WhatsNewURL2 = "https://docs.microsoft.com/en-us/windows/whats-new/whats-new-windows-10-version-1803"
)

# Function to display custom WPF message
...
$RTB.FontSize = 14
    $FlowDocument = New-Object System.Windows.Documents.FlowDocument
        $Paragraph = New-Object System.Windows.Documents.Paragraph
            ...
            $Run.Text = "Congratulations, the upgrade to $ProductName $Version $Architecture was successful!" 
        $Paragraph.AddChild($Run)
    $FlowDocument.AddChild($Paragraph)

        If ($WhatsNewURL1.Length -gt 0)
        {
            $Paragraph = New-Object System.Windows.Documents.Paragraph
            $Paragraph.TextAlignment = "Center"
                $HyperLink1 = New-Object System.Windows.Documents.Hyperlink
                $HyperLink1.NavigateUri = $WhatsNewURL1
                $HyperLink1.AddText("See what's new in $Version")
                $HyperLink1.Add_Click({Start-Process $this.NavigateURI})
                $Paragraph.AddChild($HyperLink1)
                
                If ($WhatsNewURL2.Length -gt 0)
                {
                    $HyperLink2 = New-Object System.Windows.Documents.Hyperlink
                    $HyperLink2.NavigateUri = $WhatsNewURL2
                    $HyperLink2.AddText("See what's new in $Version for IT Pro's")
                    $HyperLink2.Add_Click({Start-Process $this.NavigateURI})
                    $Paragraph.Inlines.Add(((New-object System.Windows.Documents.Run).Text ="                  "))
                    $Paragraph.AddChild($HyperLink2)
                }
            $FlowDocument.AddChild($Paragraph) 
        }
...
# Display the message
New-WPFMessageBox -Content $RTB -Title "Upgrade Complete" -TitleFontSize 24 -TitleBackground Green -TitleTextForeground White -Sound 'Windows Notify'

# Cleanup files so they can't run again
$CurrentScriptFullPathName = $MyInvocation.MyCommand.Definition
#Remove-Item $CurrentScriptFullPathName -Force -ErrorAction SilentlyContinue
#Remove-Item "$PSScriptRoot\Call-UpgradeSuccessfulNotification.ps1" -Force -ErrorAction SilentlyContinue