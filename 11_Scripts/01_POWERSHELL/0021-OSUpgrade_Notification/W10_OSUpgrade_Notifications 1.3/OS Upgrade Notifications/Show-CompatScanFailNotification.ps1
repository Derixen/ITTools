# Script to display a custom message to the user upon failure of the Windows 10 setup compatibility scan in an OS upgrade task sequence.
...
# Read the current _SMSTSOSUpgradeActionReturnCode variable value from file
...
[int64]$Code = Get-Content "$ReadFromLocation\$FileName"

# Convert hex to decimal (for reference)
#[string]$Code = "0xC1900200"
#[string]$Hex = $Code.Replace('0x','')
#$dec = [Convert]::ToInt64($Hex,16)

# Convert decimal to hex
#[int64]$Code = 3247440398
#[int64]$Code = ""
$hex = ('{0:x}' -f $Code).ToUpper()
$hex = "0x" + $hex

# List of known setup errors for compat scan
$CompatErrorTable = @{
    0xC1900208 = "Actionable compatibility issue"
    0xC1900204 = "Migration choice not available"
    0xC1900200 = "Does not meet system requirements for Windows 10"
    0xC190020E = "Insufficient free disk space"
}

# Get the friendly error description if it exists
If ($Hex -in $CompatErrorTable.Keys)
{
    $CommonError = ($CompatErrorTable.GetEnumerator() | where {$_.Name -match [int32]$Hex}).Value
}
Else
{
    $CommonError = "Unknown"
...
# Display the message
$Message = "The upgrade to Windows 10 x64 1803 could not complete. The compatibility scan returned the following error code:&#10;&#10;$hex ($CommonError)&#10;&#10;Please contact Helpdesk for further assistance."
New-WPFMessageBox -Content $Message -ContentTextForeground Black -ContentFontWeight DemiBold -ContentFontSize 16 -Title "Compatibility Scan Failure!" -TitleFontSize 24 -TitleBackground Red -TitleTextForeground White -Sound 'Windows Exclamation'