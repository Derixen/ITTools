...
--Short summary of the status of the give baseline
SELECT CIProp.DisplayName AS 'DCM Baseline Name'
	,SNames.StateName
	,COUNT(CCS.ComplianceState) AS 'Compliance State Count'

FROM dbo.v_BaselineTargetedComputers AS BTC
	INNER JOIN dbo.v_ConfigurationItems AS CI ON CI.CI_ID = BTC.CI_ID
	INNER JOIN dbo.v_CICurrentComplianceStatus AS CCS ON CCS.CI_ID = CI.CI_ID AND CCS.ResourceID = BTC.ResourceID
	INNER JOIN dbo.v_LocalizedCIProperties_SiteLoc AS CIProp ON CIProp.CI_ID = CI.CI_ID
	INNER JOIN dbo.v_StateNames AS SNames ON CCS.ComplianceState = SNames.StateID

WHERE (CIProp.DisplayName = @CBName) 
	AND (SNames.TopicType = 401)

GROUP BY CIProp.DisplayName
	,CCS.ComplianceState
	,SNames.StateName

--Detailed list of the Baseline
SELECT CIProp.DisplayName AS 'DCM Baseline Name'
	,SNames.*
...
FROM dbo.v_BaselineTargetedComputers AS BTC
	...
	INNER JOIN dbo.v_LocalizedCIProperties_SiteLoc AS CIProp ON CIProp.CI_ID = CI.CI_ID
	INNER JOIN dbo.v_StateNames AS SNames ON CCS.ComplianceState = SNames.StateID
	INNER JOIN dbo.v_R_System AS Sys ON Sys.ResourceID = CCS.ResourceID
